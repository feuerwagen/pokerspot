...
duration:h||700})});f[j](d)}else{if(e.collapsible&&f)a.toggle();else{b.hide();a.show()}i(true)}b.prev().attr("aria-expanded","false").attr("tabIndex","-1").blur();a.prev().attr("aria-expanded","true").attr("tabIndex","0")/* HACK: .focus()*/},_completed:function(a){var b=this.options;this.running=a?0:--this.running;if(!this.running){b.clearStyle&&this.toShow.add(this.toHide).css({height:"",overflow:""});this.toHide.removeClass("ui-accordion-content-active");this._trigger("change",null,this.data)}}});c.extend(c.ui.accordion,
...
f[0]&&e.shiftKey){g.focus(1);return false}}});c([]).add(d.find(".ui-dialog-content :tabbable:first")).add(d.find(".ui-dialog-buttonpane :tabbable:first")).add(d).filter(":first").focus();a._trigger("open");a._isOpen=true;return a}},
_createButtons:function(a){ var b=this, d=false, e=c("<div></div>").addClass("ui-dialog-buttonpane ui-widget-content ui-helper-clearfix");b.uiDialog.find(".ui-dialog-buttonpane").remove(); typeof a==="object"&&a!==null&&c.each(a,function(){return!(d=true)}); if(d){ c.each(a,function(g,f){ if(f.type=='button') g=c('<button type="button"></button>').text(g).appendTo(e); else g=c('<a href="#"></a>').attr('title', g).text(g).appendTo(e); c.each(f,function(h,i){ switch(h){ case'call': g.click(function(){g.button('disable');i.apply(b.element[0],arguments);return false;}); break; case'class': g.addClass(i); break; case'id': case'href': g.attr(h,i); break;} }); /*if(f.type=='button')*/ c.fn.button&&g.button() }); e.appendTo(b.uiDialog) } },
